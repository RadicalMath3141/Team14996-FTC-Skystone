    ...
    public Trajectory toTrajectory (SkystonePosition.Positions skystonePosition){
        if(redAlliance){
            if(skystonePosition == SkystonePosition.Positions.LEFT){
                return robot.drive().trajectoryBuilder().splineTo(new Pose2d(-47,-39,Math.toRadians(180))).splineToConstantHeading(new Pose2d(-53,-22,Math.toRadians(180))).splineToConstantHeading(new Pose2d(-68,-22,Math.toRadians(180))).build();
            } else if(skystonePosition == SkystonePosition.Positions.MIDDLE){
                return robot.drive().trajectoryBuilder().splineTo(new Pose2d(-39,-39,Math.toRadians(180))).splineToConstantHeading(new Pose2d(-45,-22,Math.toRadians(180))).splineToConstantHeading(new Pose2d(-60,-22,Math.toRadians(180))).build();
            } else if(skystonePosition == SkystonePosition.Positions.RIGHT){
                return robot.drive().trajectoryBuilder().splineTo(new Pose2d(-31,-39,Math.toRadians(180))).splineToConstantHeading(new Pose2d(-37,-22,Math.toRadians(180))).splineToConstantHeading(new Pose2d(-52,-22,Math.toRadians(180))).build();
            }
